...
async def test_independent_tasks_run_in_parallel(store, tmp_path):
    ...
    # Use min_score=0.0 so quality gate passes on first attempt (scoring call
    # still happens but doesn't trigger retries).
    ...
    # 2 worker calls + 2 quality gate scoring calls = 4 total
    assert len(execution_times) == 4
    # The first two calls (worker executions) should start concurrently
