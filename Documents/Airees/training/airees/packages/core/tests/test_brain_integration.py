...
def _make_score_response(score: int = 9) -> MagicMock:
    """Build a mock quality-gate scoring response."""
    return _make_text_response(f'{{"score": {score}, "feedback": "Good"}}')
...
async def test_full_brain_loop(store, tmp_path):
    ...
    The orchestrator makes exactly 7 calls to the router:
      ...
      3. _score_output     — Quality gate scores Research output
      4. _execute_worker   — Worker runs Build task (depends on Research)
      5. _score_output     — Quality gate scores Build output
      6. _evaluate          — Brain evaluates via evaluate_result tool
    """
    ...
    # Call 3: Quality gate scores Worker 1
    score1_response = _make_score_response()
    ...
    # Call 5: Quality gate scores Worker 2
    score2_response = _make_score_response()

    # Call 6: Brain evaluates (evaluate_result tool call)
    ...
    # Verify the router was called exactly 7 times
    # (intent + plan + 2 workers + 2 quality scores + eval)
    assert mock_router.create_message.call_count == 7
...
async def test_brain_loop_with_adapt_iteration(store, tmp_path):
    ...
    Call flow:
      ...
      3. _score_output     — Quality gate scores the task
      4. _evaluate (iter 0) — Brain says "adapt"
      5. _evaluate (iter 1) — Brain says "satisfied" (no new ready tasks)
    """
    mock_router = AsyncMock()
    ...
    score_response = _make_score_response()
    ...
    goal = await store.get_goal(goal_id)
    assert goal["status"] == "completed"
    assert mock_router.create_message.call_count == 6
    assert orch.state_machine.state == BrainState.IDLE
