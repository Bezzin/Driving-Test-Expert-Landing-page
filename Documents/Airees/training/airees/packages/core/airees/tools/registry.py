...
class TrustLevel(Enum):
    """Trust classification for tool sources."""
    BUILTIN = "builtin"
    MCP = "mcp"
    CLI = "cli"
...
class ToolRegistry:
    ...
    def filter_for_role(
        self, role: str, access_config: dict[str, dict[str, list[str]]]
    ) -> list[ToolDefinition]:
        """Return tools filtered by role-based allow/block lists.

        Args:
            role: The agent role to filter for.
            access_config: Mapping of role -> {"allow": [...], "block": [...]}.
                           Use ["*"] in allow to permit all tools.

        Returns:
            List of ToolDefinition objects the role is permitted to use.
        """
        role_config = access_config.get(role)
        if role_config is None:
            return []

        allow = role_config.get("allow", [])
        block = set(role_config.get("block", []))

        if "*" in allow:
            return [t for t in self._tools.values() if t.name not in block]

        return [
            self._tools[name]
            for name in allow
            if name in self._tools and name not in block
        ]
